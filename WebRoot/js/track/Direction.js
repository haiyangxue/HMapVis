...
createDirection = function(line,position,forEachSegment, size, color) {
	if (line instanceof OpenLayers.Geometry.MultiLineString) {
		//TODO
	} else if (line instanceof OpenLayers.Geometry.LineString) {
		return createLineStringDirection(line,position,forEachSegment, size, color);
	} else {
		return [];
	}
};

createLineStringDirection = function(line,position, forEachSegment, size, color){
	...
	for (var i=0;i<segs.length ;i++ )	{
		points = points.concat(createSegDirection(segs[i],position, size, color) );
	}
	return points;
};

createSegDirection = function(seg,position, size, color) {
	...
	for (var i=0;i<positions.length;i++ ){
		...
		var ptFeature = new OpenLayers.Feature.Vector(pt,{angle:segBearing,size:size,color:color}); 
