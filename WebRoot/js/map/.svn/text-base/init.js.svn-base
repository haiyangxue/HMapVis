...
var heatmap_place_loc;
var event_relation_json={
		addNewEventFlag:false,
		addExistingEventFlag:false,
		queryResultIsZero :false,
		Event_id : -1,
		relatedEvent_id : -1,
		relatedEventtype_id :-1}; 

var language; // 0 --> English; 1 --> Chinese

function init(){
	var indicatorS = $("#timeline-indicatorS");
	indicatorS.css("left",$(window).width()/2-95);
...
//	 map.addLayer(wms_layer);
	...
	//热度图
	 testData={
           max: 46,//
           data: [
                  {lat: 33.5363, lon:-117.044, count: 1},
  ...
  //点层
	vectorLayer = new OpenLayers.Layer.Vector("Simple Geometry", {
        styleMap: new OpenLayers.StyleMap({'default':{
            strokeColor: "#c9c9c9",//点的边的颜色
            strokeOpacity: 0,//点的边的透明度
            strokeWidth: 5,//点的边的半径
            fillColor: "#c9c9c9",//点的中心的颜色
            fillOpacity: 0.1,//点的中心的透明度
            pointRadius: 8,//点的中心的半径
            pointerEvents: "visiblePainted",
            // label with \n linebreaks
            label : "${count}",//点旁边文字显示的内容包括name和age
            
            fontColor: "${favColor}",//点旁边文字颜色
            fontSize: "15px",//点旁边文字大小
...
//	},"json");
	...
	models.fetch({  
		...
		success : function() {
			...
			//获得数据后的操作  
			$("#timeline").timeline({zoomlevel:22,'stdata':models});
