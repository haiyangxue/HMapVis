...
public class QueryEventsAction extends BaseAction{
	...
	public String fetchEventList(){
		try{
		//这里添加修改的部分   后面加注释的都是自己加上去的

			eventlist = eventService.findEvents("event_name",event.getEvent_name(), "type_id", event.getType_id(), "dynasty_id",event.getDynasty_id(), "place_id",event.getPlace_id());
